...
publicRoutes.get('/v1/catalog', asyncHandler(async (req, res) => {
    ...
    if (languagesError)
        throw new HttpError(500, 'languages_fetch_failed', languagesError.message);
    const { data: courses, error: coursesError } = await supabaseAdmin
        ...
        .order('sort_order');
    if (coursesError)
        throw new HttpError(500, 'courses_fetch_failed', coursesError.message);
...
publicRoutes.get('/v1/lid/manifest', asyncHandler(async (req, res) => {
    ...
    if (!manifest)
        throw new HttpError(404, 'lid_manifest_not_published');
    res.json(manifest);
}));
