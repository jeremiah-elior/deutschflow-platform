...
import { buildCourseLevelManifest, ChapterAssetInput, ChapterInput, CourseInput, createChapter, createCourse, createLevel, LanguageInput, LevelInput, listCourses, listLanguages, saveChapterAsset, upsertLanguage } from '../services/courseService.js';
...
import { AssetPatchInput, CategoryInput, CategoryPatchInput, createCategory, createNote, createQuiz, createSeries, createVideo, createVocabulary, deleteCategory, deleteChapter, deleteChapterAsset, deleteCourse, deleteLevel, deleteNote, deleteQuiz, deleteSeries, deleteVideo, deleteVocabulary, listAdminContentOptions, listChapterAssets, NoteInput, NotePatchInput, QuizInput, QuizPatchInput, SeriesInput, SeriesPatchInput, updateCategory, updateChapter, updateChapterAsset, updateCourse, updateLevel, updateNote, updateQuiz, updateSeries, updateVideo, updateVocabulary, VocabularyInput, VocabularyPatchInput, VideoInput, VideoPatchInput, CoursePatchInput, LevelPatchInput, ChapterPatchInput } from '../services/contentCrudService.js';
...
adminRoutes.get('/v1/admin/chapters', asyncHandler(async (req, res) => {
    res.json({ chapters: await listAdminChapters(req.query) });
}));
adminRoutes.get('/v1/admin/vocabulary', asyncHandler(async (req, res) => {
    res.json({ vocabulary: await listAdminVocabulary(req.query) });
}));
adminRoutes.get('/v1/admin/notes', asyncHandler(async (req, res) => {
    res.json({ notes: await listAdminNotes(req.query) });
}));
adminRoutes.get('/v1/admin/videos', asyncHandler(async (req, res) => {
    res.json({ videos: await listAdminVideos(req.query) });
}));
adminRoutes.get('/v1/admin/quiz', asyncHandler(async (req, res) => {
    res.json({ quiz: await listAdminQuiz(req.query) });
}));
...
adminRoutes.get('/v1/admin/chapter-assets', asyncHandler(async (req, res) => {
    res.json({ assets: await listChapterAssets(req.query) });
}));
