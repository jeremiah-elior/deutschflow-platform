...
async function tableCount(table) {
    ...
    if (error)
        throw new HttpError(500, `${table}_count_failed`, error.message);
    return count ?? 0;
}
export async function getAdminOverview() {
    const [languages, courses, levels, chapters, categories, series, notes, transcripts, vocabulary, videos, quiz, chapterAssets, lidCatalogs, lidCards, releases] = await Promise.all([
    ...
    const { data: recentChapters, error: recentError } = await supabaseAdmin
        ...
        .limit(6);
    if (recentError)
        throw new HttpError(500, 'recent_chapters_failed', recentError.message);
    const { data: latestReleases, error: releaseError } = await supabaseAdmin
        ...
        .limit(6);
    if (releaseError)
        throw new HttpError(500, 'latest_releases_failed', releaseError.message);
    return {
        counts: {
            languages,
            courses,
...
export async function listAdminChapters(query) {
    ...
    if (query.level)
        request = request.eq('level.slug', query.level);
    ...
    if (error)
        throw new HttpError(500, 'chapters_fetch_failed', error.message);
    ...
    return search ? rows.filter((row) => JSON.stringify(row).toLowerCase().includes(search)) : rows;
}
export async function listAdminVocabulary(query) {
    ...
    const { data, error } = await request;
    if (error)
        throw new HttpError(500, 'vocabulary_fetch_failed', error.message);
...
export async function listAdminNotes(query) {
    ...
    const { data, error } = await request;
    if (error)
        throw new HttpError(500, 'notes_fetch_failed', error.message);
    return (data ?? []).map((item) => ({
        id: item.id,
        legacyId: item.legacy_id,
...
export async function listAdminVideos(query) {
    let request = supabaseAdmin
        .from('chapter_videos')
    ...
    const { data, error } = await request;
    if (error)
        throw new HttpError(500, 'videos_fetch_failed', error.message);
    return (data ?? []).map((item) => ({
        ...item,
        chapter: item.chapter ? {
            id: item.chapter.id,
...
export async function listAdminQuiz(query) {
    let request = supabaseAdmin
    ...
    const { data, error } = await request;
    if (error)
        throw new HttpError(500, 'quiz_fetch_failed', error.message);
    return (data ?? []).map((item) => ({
        ...item,
        chapter: item.chapter ? {
            id: item.chapter.id,
...
export async function listAdminCategoriesAndSeries() {
    ...
    if (categoryError)
        throw new HttpError(500, 'categories_fetch_failed', categoryError.message);
    if (seriesError)
        throw new HttpError(500, 'series_fetch_failed', seriesError.message);
