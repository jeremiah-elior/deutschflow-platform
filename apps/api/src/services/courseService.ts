...
export async function upsertLanguage(input: z.infer<typeof LanguageInput>) {
  ...
  if (error) throw new HttpError(500, 'language_save_failed', error.message);
  return data;
}
...
export async function createCourse(input: z.infer<typeof CourseInput>) {
  ...
  if (error) throw new HttpError(500, 'course_create_failed', error.message);
  return data;
}

export async function createLevel(input: z.infer<typeof LevelInput>) {
  ...
  if (error) throw new HttpError(500, 'level_create_failed', error.message);
  return data;
}

export async function createChapter(input: z.infer<typeof ChapterInput>) {
  ...
  if (error) throw new HttpError(500, 'chapter_create_failed', error.message);
  return data;
}
...
export async function buildCourseLevelManifest(courseSlug: string, levelSlug: string, languageCode: string) {
  const { data: course, error: courseError } = await supabaseAdmin
    ...
    .eq('is_active', true)
    .single();
  if (courseError || !course) throw new HttpError(404, 'course_not_found');
  ...
  const { data: level, error: levelError } = await supabaseAdmin
    ...
    .eq('is_active', true)
    .single();
  if (levelError || !level) throw new HttpError(404, 'level_not_found');
  ...
  const { data: chapters, error: chapterError } = await supabaseAdmin
    ...
    .eq('is_active', true)
    .order('number');
  if (chapterError) throw new HttpError(500, 'chapters_fetch_failed', chapterError.message);

  const version = new Date().toISOString();
  const manifest: CourseLevelManifest = {
    module: 'course_level',
    schemaVersion: 1,
    courseSlug,
    levelSlug,
  ...
  if (releaseError) throw new HttpError(500, 'release_save_failed', releaseError.message);
