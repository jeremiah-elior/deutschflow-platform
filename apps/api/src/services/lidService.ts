...
export async function listLiDCatalogs() {
  ...
  if (error) throw new HttpError(500, 'lid_catalogs_fetch_failed', error.message);
  return data;
}
...
export async function getActiveLiDCatalog() {
  const { data, error } = await supabaseAdmin
    ...
    .limit(1)
    .maybeSingle();
  if (error) throw new HttpError(500, 'active_lid_catalog_fetch_failed', error.message);
  return data;
}
...
export async function importLiDJson(input: z.infer<typeof ImportLiDJsonInput>, actorId?: string) {
  ...
  if (catalogError || !catalog) throw new HttpError(500, 'lid_catalog_create_failed', catalogError?.message);
  ...
  for (let i = 0; i < rows.length; i += 500) {
    ...
    if (error) throw new HttpError(500, 'lid_cards_insert_failed', error.message);
  }
...
export async function saveLiDAsset(input: z.infer<typeof LiDAssetInput>) {
  ...
  if (error) throw new HttpError(500, 'lid_asset_save_failed', error.message);
  return data;
}

function getLocalizedPack(cards: any[], lang: string) {
  ...
  return {
    ...
    cards: cards.map((card) => ({
      ...
      choices: (card.choices_json ?? []).map((choice: any) => ({
        key: choice.key,
        de: choice.de,
        en: choice.en ?? null,
...
export async function publishLiDManifest(languageCode = 'te') {
  ...
  if (!catalog) throw new HttpError(404, 'no_active_lid_catalog');
  ...
  const { data: cards, error: cardsError } = await supabaseAdmin
    ...
    .order('part')
    .order('number');
  if (cardsError) throw new HttpError(500, 'lid_cards_fetch_failed', cardsError.message);
  ...
  if (assetsError) throw new HttpError(500, 'lid_assets_fetch_failed', assetsError.message);

  const media: Record<string, any> = {};
  const samples: Record<string, any> = {};
  const imageItems: Record<string, string> = {};
  ...
  for (const asset of assets ?? []) {
    ...
    if (asset.asset_type === 'image') imageItems[asset.key] = asset.storage_path.split('/').pop();
    if (asset.asset_type === 'intro_video') media.introVideo = descriptor;
    if (asset.asset_type === 'tips_audio') media[`tipsAudio_${asset.language_code ?? 'default'}`] = descriptor;
    if (asset.asset_type === 'audio_track') media[`audioTrack_${asset.key}`] = descriptor;
    if (asset.asset_type === 'sample_json') samples[asset.key] = descriptor;
  ...
  if (releaseError) throw new HttpError(500, 'lid_release_save_failed', releaseError.message);
...
export async function getPublishedLiDManifest(languageCode = 'te') {
  const { data, error } = await supabaseAdmin
    ...
    .limit(1)
    .maybeSingle();
  if (error) throw new HttpError(500, 'lid_manifest_fetch_failed', error.message);
  if (!data) return null;
