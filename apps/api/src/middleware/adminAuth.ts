...
export async function requireAdmin(req: Request, _res: Response, next: NextFunction) {
  try {
    ...
    if (profileError) throw new HttpError(500, 'admin_profile_lookup_failed', profileError.message);
    if (!profile?.is_active) throw new HttpError(403, 'admin_access_denied');
